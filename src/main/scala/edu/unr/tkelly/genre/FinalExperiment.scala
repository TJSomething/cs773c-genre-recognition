...
object FinalExperiment extends App {
  ...
  //  feature vectors
  def framePassthrough[B[_] <: GenTraversable[_]](frames: B[Frame])
  (implicit bf: CanBuildFrom[Nothing, B[FrameVector], B[B[FrameVector]]]): B[B[FrameVector]] = {
    val builder = bf()
    def helper() = {
      (for (frame <- frames) yield
    	for (segment <- frame.asInstanceOf[Frame];
    	     feature <- segment) yield feature).asInstanceOf[B[FrameVector]]
    }
    builder += helper()
    builder.result
  }
  ...
  //  vectors
  def frameSplit[B[_] <: GenTraversable[_]](frames: B[Frame])
      (implicit bf: CanBuildFrom[Nothing, B[FrameVector], B[B[FrameVector]]]): B[B[FrameVector]] = {
    val builder = bf()
    ...
    def sliceFrames[T, B[_] <: GenTraversable[_]](start: Int, end: Int): B[FrameVector] = {
    	(for (frame <- frames) yield
    	    for (segment <- frame.asInstanceOf[Frame];
    	         feature <- segment.slice(start, end)) yield feature)
  ...
  def splitSong(s: Song, frameLength: Int): SortedMap[Time, Frame] = {
    ...
    (for (
        ...
        (for (
          ...
          (for (featureIndex <- 0 until 25) yield
	          if (featureIndex < 12)
  ...
  val splitSongSets =
    // For every cross-validation,
      ...
      for (songSet <- List(training, testing)) yield for (song <- songSet) yield (Util.songToShortString(song),
        for (frameLength <- 1 to maxFrameLength) yield splitSong(song, frameLength))
    }
  // Note that the dimensions on this are:
  // fold, training+testing, song, title*(start time -> frame)
  
  // For every cross-validation,
  for (List(training, testing) <- splitSongSets) {
    ...
    for (splitMethod <- splitInfo) {
      println("splitways:" + splitMethod.featureNames)
      // Format frames for clustering
      val framesByFrameLengthThenSong =
        (training map (song => song._2)) transpose
      val framesByFrameLength = 
        framesByFrameLengthThenSong map (songs => songs.flatMap(_.values))
      val frameVectorsByFrameLength =
          (framesByFrameLength map { frames: Iterable[Frame] =>
            splitMethod.splitter(shuffle(frames.toSeq))
          }).toSeq
      ...
      val instancesByFrameLength_Type =
        for (frameVectorsByFeatureType <- frameVectorsByFrameLength) yield 
      ...
      val clusterersByFrameLength_Type =
        for ((instancesByType, frameIndex) <- instancesByFrameLength_Type.zipWithIndex) yield 
          for (instances <- instancesByType) yield 
